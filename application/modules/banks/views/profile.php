...
<? } ?></div>             
	...
	<div class="row well" style="margin-top:2%">
    	...
    	<div class="col-md-6">  
         <address>
        <?php if($row->address1){echo  '<strong><h4>Address:</h4></strong> '. $row->address1.'<br />';} ?>
        <?php if($row->address2){echo  $row->address2.'<br />';} ?>
        <?php if($row->postCode){echo  $row->postCode.'<br />';} ?>
        </address>
        
        <abbr title="Phone"><span class="glyphicon glyphicon-earphone"></span></abbr> <a href="tel: +44<?php echo ltrim($row->telephone, '0') ;?>"> <?php echo $row->telephone ;?></a><br /><br />
